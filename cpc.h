...
public:
	cpc(size_t hscale = 2, size_t wscale = 2);
	void parse_opts(int argc, char ** argv);
	void initialize();
	void run();

	virtual uint32_t readmem(uint16_t address) const override;
	virtual void writemem(uint16_t address, uint8_t value) override;
	virtual uint8_t readio(uint16_t address) const override;
...
private:
	...
	static constexpr size_t BLOCKS = 8;
	static constexpr size_t BLOCK_SIZE = 16387;
	static constexpr size_t LROM_SIZE = 16387;

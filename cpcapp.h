...
public:
  CtrlFrame(const wxString &title, const wxPoint &pos, const wxSize &size,
            CPCApp &app);
...
enum { BTN_OPEN_FLOPPY = wxID_HIGHEST + 1, BTN_RESET };

wxBEGIN_EVENT_TABLE(CtrlFrame, wxFrame)
    EVT_BUTTON(BTN_OPEN_FLOPPY, CtrlFrame::OnOpenFloppy)
        EVT_BUTTON(BTN_RESET, CtrlFrame::OnReset) EVT_CLOSE(CtrlFrame::OnClose)
            wxEND_EVENT_TABLE()

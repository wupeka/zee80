...
constexpr size_t zx48k::MEMORY_SIZE;

namespace {
constexpr size_t COLS = 352;
constexpr size_t LINES = 296;
constexpr size_t TOP_BORDER_LINES = 48;
constexpr size_t BOTTOM_BORDER_LINES = 56;
constexpr size_t LEFT_BORDER_COLS = 48;
constexpr size_t RIGHT_BORDER_COLS = 48;
...
zx48k::zx48k(size_t hscale, size_t wscale) :
...
uint32_t zx48k::readmem(uint16_t address) const {
...
uint8_t zx48k::readio(uint16_t address) const {
	uint8_t out = 0;
...
void zx48k::scanline(int y) {
	...
	if ((y < TOP_BORDER_LINES) || (y >= LINES - BOTTOM_BORDER_LINES) ) {
		set_line(y, zxpalette[border]);
	} else {
		draw_line(0, LEFT_BORDER_COLS - 1, y, zxpalette[border]);
		for (int xx = 0; xx < 32; xx++) {
			...
			for (int i = 0; i < 8; i++) {
				const bool lit = memory[addr] & (1 << (7-i));
				set_pixel(TOP_BORDER_LINES + 8*xx + i, y, zxpalette[lit ? ink : paper]);
			}
		}
		// TODO unloopize it
		draw_line(COLS - RIGHT_BORDER_COLS, COLS - 1, y, zxpalette[border]);
	}
